<x-layout>
  <div class="flex flex-col w-full h-max items-center gap-5">
   ...
   <form method='POST' action='/category/{{$category->id}}' class="w-full 2xl:w-6/12 rounded-xl border-2 border-white bg-transparent shadow-lg p-10 mb-10 mt-3 h-full" id="submit">
    @csrf
    @method('PUT')
    <div class="mb-6">
      <label for="name">
      ...
      <input type="text" class="border border-gray-200 rounded p-2 w-full" name="name"
          value="{{$category->name}}" autofocus placeholder="MoMo" />
      @error('name')
