            ...
            <div class="flex flex-col gap-2 mt-2 max-h-fit" id="new">
                @foreach ($pendings as $var)
                    <x-card class="justify-between">
                        ...
                        <a href="/orderitems/{{ $var->id }}/update/preparing" class="py-2 px-4 rounded-lg text-white hover:bg-amber-500 hover:text-gray-200 smooth text-xl bg-amber-600">Start</a>
                    </x-card>
                @endforeach
            </div>
            <script>
                ...
                @foreach ($cookings as $var)
                    @foreach ($items as $item)
                        @if ($item['id'] == $var['item_id'])
                            <x-card class="justify-between">
                                @include('_partials.orderItems')
                                <a href="/orderitems/{{ $var->id }}/update/prepared" class="py-2 px-4 rounded-lg text-white hover:bg-lime-500 hover:text-gray-200 smooth text-xl bg-lime-600">Done</a>
                            </x-card>
                        @endif
                    @endforeach
                @endforeach
